...
export default function HistoricoCompras({
  ...
  // ATUALIZADO: Agora suporta pagamento único e pagamento em massa (Pagar Tudo)
  const confirmarEnvioPix = async () => {
    setProcessandoPix(true);
    try {
      if (vendaPix.isBulk) {
        // Pagar Múltiplos Pedidos
        const promessas = vendaPix.ids.map((idDaVenda) => {
          const vendaRef = doc(db, "vendas", idDaVenda);
          return updateDoc(vendaRef, { aguardandoConfirmacao: true });
        });
        ...
        setCompras((atual) =>
          atual.map((c) =>
            c.id === vendaPix.id ? { ...c, aguardandoConfirmacao: true } : c,
          ),
        );
      }
  ...
  // NOVO: Calcula apenas o que está livre para pagar agora (ignora o que já está em análise)
  const comprasPendentesAcao = compras.filter(
    (c) => !c.pago && !c.aguardandoConfirmacao,
  );
  const totalPagarAgora = comprasPendentesAcao.reduce(
    (acc, c) => acc + Number(c.total),
    0,
  );

  // NOVO: Função que abre o modal configurado para "Pagar Tudo"
  const handlePagarTudoPix = () => {
    setVendaPix({
      isBulk: true,
      ids: comprasPendentesAcao.map((c) => c.id),
      total: totalPagarAgora,
    });
  };

  // Lógica que filtra as compras exibidas com base no botão clicado
  ...
  return (
    <div className="w-full flex flex-col gap-6 relative animate-fade-in-up">
          ...
          {sucessoPix ? (
            <div className="bg-white w-full max-w-sm rounded-3xl p-8 flex flex-col items-center justify-center shadow-2xl animate-fade-in-up">
              ...
              <p className="text-gray-500 text-center font-medium">
                A loja vai conferir o seu pagamento e liberar o pedido.
              </p>
            </div>
          ) : (
              ...
              <div className="p-6 flex flex-col items-center bg-white -mt-4 rounded-t-3xl">
                ...
                <div className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-5 mb-4 text-center">
                  ...
                  <p className="text-3xl font-extrabold text-emerald-600">
                    R$ {Number(vendaPix.total).toFixed(2).replace(".", ",")}
                  </p>
                </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
          <div>
            <p className="text-amber-800 font-bold text-sm">
              Você possui pagamentos pendentes:
            </p>
          ...
          {/* NOVO: Botão Pagar Tudo */}
          {totalPagarAgora > 0 && (
            <button
              onClick={handlePagarTudoPix}
              className="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-white font-bold py-3 px-6 rounded-xl transition-all shadow-md flex justify-center items-center gap-2"
            >
              💠 Pagar Tudo (R$ {totalPagarAgora.toFixed(2).replace(".", ",")})
            </button>
          )}
        </div>
      )}
      ...
      {/* LISTA DE COMPRAS */}
      {carregando ? (
        <p className="text-center text-gray-500 mt-10 animate-pulse font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
          {comprasFiltradas.map((compra) => (
            <div
              key={compra.id}
              className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 flex flex-col justify-between"
            >
              <div>
                <div className="flex justify-between items-start mb-4 border-b border-gray-100 pb-3">
                  <div className="flex flex-col gap-1.5">
                    <span className="text-xs text-gray-400 font-medium">
                      {new Date(compra.data).toLocaleString("pt-BR")}
                    </span>
                    {compra.metodoPagamento === "pix" ? (
                      <span className="w-fit text-[10px] font-bold uppercase px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-600 border border-indigo-100">
                        💠 Pix
                      </span>
                    ) : compra.metodoPagamento === "vr" ? (
                      <span className="w-fit text-[10px] font-bold uppercase px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-600 border border-emerald-100">
                        💳 VR
                      </span>
                    ) : (
                      <span className="w-fit text-[10px] font-bold uppercase px-2 py-0.5 rounded-md bg-gray-100 text-gray-500 border border-gray-200">
                        Não informado
                      </span>
                    )}
                  </div>
